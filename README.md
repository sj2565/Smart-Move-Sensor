...
## 1. 프로젝트 소개
이동형 환경 및 충격 감지 시스템은 STM32 기반의 IoT 센서 시스템을 활용하여 이동 중 발생할 수 있는 환경 정보 수집과 충격(포트홀) 감지를 수행하고, 이를 실시간으로 위치와 함께 시각화하는 시스템. <br>
가속도 센서를 이용해 충격 이벤트를 감지하고, 온습도/공기질 센서를 통해 실외 환경 상태를 측정하며 GPS 모듈을 통해 위치 데이터를 수집. <br>
수집된 데이터는 ESP32를 통해 MQTT로 전송되며, 브라우저 기반 Google Maps 시각화 페이지에서 실시간으로 확인 가능. <br>
현재는 Wi-Fi 기반으로 작동하며, 향후 배터리 전원 및 저전력 구조를 도입하여 야외 이동형 장비로의 확장 가능성을 고려 중. <br>
해당 시스템은 도로 위 위험 요소(예: 포트홀)의 위치 파악 및 환경 상태 분석에 활용 가능. <br>
...
## 7. 향후 개선할 점
+ 수집된 센서 데이터를 기반으로 이동 패턴 분석 및 포트홀 위험도 예측 등 지능형 판단이 가능한 AI모델 도입.
...
-------------------------------------------------------------------------------------
고려해 볼 만한 사항
+ 이동평균필터 외에 다양한 DSP(디지털 신호 처리) 적용.
+ 단순한 HTML이 아닌 서버와 데이터베이스 활용.
